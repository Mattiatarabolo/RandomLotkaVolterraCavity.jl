...
@testset "RandomLotkaVolterraCavity" begin
    ...
    @testset "Cavity and Marginal messages" begin
        ...
        cav_q0 = CavityFP_q0(1, 2, :zero, 0.0, 0.0, rng)
        @test cav_q0.i == 1
        @test cav_q0.j == 2
        @test cav_q0.x == 0.0
        @test cav_q0.chi == 0.0

        cav_q0_rand = CavityFP_q0(1, 2, :random, 0.0, 0.0, rng)
        @test cav_q0_rand.i == 1
        @test cav_q0_rand.j == 2
        @test cav_q0_rand.x == rand(rng_ref)
        @test cav_q0_rand.chi == 0.0

        cav_q0_custom = CavityFP_q0(1, 2, :custom, 0.8, 0.3, Xoshiro(BASE_SEED))
        @test cav_q0_custom.i == 1
        @test cav_q0_custom.j == 2
        @test cav_q0_custom.x == 0.8
        @test cav_q0_custom.chi == 0.3

        marg_q0 = MarginalFP_q0(1, :zero, 0.0, 0.0, rng)
        @test marg_q0.i == 1
        @test marg_q0.x == 0.0
        @test marg_q0.chi == 0.0
        
        marg_q0_rand = MarginalFP_q0(1, :random, 0.0, 0.0, rng)
        @test marg_q0_rand.i == 1
        @test marg_q0_rand.x == rand(rng_ref)
        @test marg_q0_rand.chi == 0.0

        marg_q0_custom = MarginalFP_q0(1, :custom, 0.8, 0.3, rng)
        @test marg_q0_custom.i == 1
        @test marg_q0_custom.x == 0.8
        @test marg_q0_custom.chi == 0.3
        ...
        node_q0 = NodeFP_q0(1, [2], :zero, 0.7, 0.2, rng)
        @test node_q0.i == 1
        ...
        node_q0 = NodeFP_q0(1, [2], :random, 0.7, 0.2, rng)
        @test node_q0.i == 1
        @test node_q0.neighs == [2]
        @test node_q0.neighs_idx[2] == 1
        @test typeof(node_q0.cavs[1]) == CavityFP_q0{Deterministic, Int64, Float64}
        ...
        node_q0 = NodeFP_q0(1, [2], :custom, 0.7, 0.2, rng)
        @test node_q0.i == 1
        @test node_q0.neighs == [2]
        @test node_q0.neighs_idx[2] == 1
        @test typeof(node_q0.cavs[1]) == CavityFP_q0{Deterministic, Int64, Float64}
    ...
    @testset "Populations" begin
        ...
        pop_q0_zero = PopFP_q0(2, :zero, 0.0, 0.0, rng)  # q0 variant vector checks
        @test pop_q0_zero.x_pop == zeros(2)
        @test pop_q0_zero.chi_pop == zeros(2)

        pop_q0_rand = PopFP_q0(2, :random, 0.0, 0.0, rng)
        @test pop_q0_rand.x_pop == rand(rng_ref, 2)
        @test pop_q0_rand.chi_pop == zeros(2)

        pop_q0_custom = PopFP_q0(2, :custom, 0.3, 0.4, rng)
        @test pop_q0_custom.x_pop == fill(0.3, 2)
        @test pop_q0_custom.chi_pop == fill(0.4, 2)
    ...
    @testset "Sampling" begin
        ...
        J, Jp = sample_couplings(0.2, 0.3, 0.1, 2.0; rng=rng)  # regression for integer 

        traj, tsave, conv = run_MC(base_model, 0.05; rng=rng, idxs_tsave=collect(0:2))  # deterministic graph path
        @test size(traj) == (2, 3)
        @test tsave == 0.05 .* collect(0:2)
        @test conv
        @test minimum(traj) >= base_model.lam
        
        traj_dis, tsave_dis, conv_dis = run_MC(base_model_dis, 0.05; rng=rng, idxs_tsave=collect(0:2))  # sparse disordered instance path
        @test size(traj_dis) == (2, 3)
        @test tsave_dis == 0.05 .* collect(0:2)
        @test conv_dis
        @test minimum(traj_dis) >= base_model_dis.lam

        traj_fc, tsave_fc, conv_fc = run_MC(base_model_dis_fc, 0.05; rng=rng, idxs_tsave=collect(0:2))  # fully-connected disordered integration
        @test size(traj_fc) == (2, 3)
        @test tsave_fc == 0.05 .* collect(0:2)
        @test conv_fc
        @test minimum(traj_fc) >= base_model_dis_fc.lam
    end

    @testset "ODE sampler" begin
        rng = Xoshiro(BASE_SEED)
        rng_ref = copy(rng)
        tsave = range(0.0, step=0.05, length=3)

        traj, tsave_out, conv, t_eq = run_MC_ODE(base_model, 0.05; rng=rng, tsave=collect(tsave))
        @test size(traj) == (2, 3)
        @test tsave_out == collect(tsave)
        @test conv
        @test t_eq ≈ tsave[end]

        traj_dis, tsave_dis, conv_dis, t_eq_dis = run_MC_ODE(base_model_dis, 0.05; rng=rng, tsave=collect(tsave))
        @test size(traj_dis) == (2, 3)
        @test tsave_dis == collect(tsave)
        @test conv_dis
        @test t_eq_dis ≈ tsave[end]

        traj_fc, tsave_fc, conv_fc, t_eq_fc = run_MC_ODE(base_model_dis_fc, 0.05; rng=rng, tsave=collect(tsave))
        @test size(traj_fc) == (2, 3)
    ...
    @testset "Fixed-point solvers" begin
        ...
        nodes_q0, conv_q0, diverged_q0 = run_GECaM_FP_q0(base_model, 5, 1e-3, 0.5; init_type=:custom, x0=0.2, chi0=0.05, rng=Xoshiro(BASE_SEED), regularization=1e-6)
        ...
        cav_pop_q0, marg_pop_q0, conv_dis_q0, diverged_dis_q0 = run_GECaM_FP_q0(base_model_dis, 5, 5, 1e-3, 0.5; init_type=:custom, x0=0.2, chi0=0.05, rng=Xoshiro(BASE_SEED), regularization=1e-6)
