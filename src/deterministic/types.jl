...
mutable struct CavityFP{Deterministic, I<:Integer, RT<:Real}
    ...
    psi::RT
    """
        CavityFP{Deterministic, I, RT}(i::I, j::I, init_type::Symbol, mu0::RT, q0::RT, chi0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a new fixed-point cavity with the specified node index and neighbor index.
...
$(TYPEDFIELDS)
"""
mutable struct CavityFP_BP{Deterministic, I<:Integer, RT<:Real}
    """ Node index """
    i::I
    """ Neighbor index """
    j::I
    ...
    q::RT
    """
        CavityFP_BP{Deterministic, I, RT}(i::I, j::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a new Belief Propagation (BP) fixed-point cavity with the specified node index and neighbor index.
    ...
    # Output
    - `CavityFP_BP`: Cavity with the specified node and neighbor indices.
    """
    function CavityFP_BP(i::I, j::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
        if init_type == :zero
            return new{Deterministic, I, RT}(i, j, zero(RT), zero(RT))
        elseif init_type == :random
            return new{Deterministic, I, RT}(i, j, rand(rng), rand(rng))
        elseif init_type == :custom
            return new{Deterministic, I, RT}(i, j, mu0, q0)
        end
    end
end
...
$(TYPEDFIELDS)
"""
mutable struct MarginalFP_BP{Deterministic, I<:Integer, RT<:Real}
    """ Node index """
    i::I
    """ Marginal mean abundance """
    mu::RT
    ...
    q::RT
    """
        MarginalFP_BP{Deterministic, I, RT}(i::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a new Belief Propagation (BP) fixed-point marginal with the specified node index.
    ...
    # Output
    - `MarginalFP_BP`: Marginal with the specified node index.
    """
    function MarginalFP_BP(i::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
        if init_type == :zero
            return new{Deterministic, I, RT}(i, zero(RT), zero(RT))
        elseif init_type == :random
            return new{Deterministic, I, RT}(i, rand(rng), rand(rng))
        elseif init_type == :custom
            return new{Deterministic, I, RT}(i, mu0, q0)
        end
    end
end
...
struct NodeFP{Deterministic, I<:Integer, RT<:Real}
    ...
    """
        NodeFP{Deterministic, I, RT}(i::I, neighs::Vector{I}, init_type::Symbol, mu0::RT, q0::RT, chi0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
...
$(TYPEDFIELDS)
"""
mutable struct NodeFP_IBMF{Deterministic, I<:Integer, RT<:Real}
    """ Node index """
    i::I
    """ Neighbors' indices """
    neighs::Vector{I}
    ...
    x::RT
    """
        NodeFP_IBMF{Deterministic, I, RT}(i::I, neighs::AbstractVector{I}, init_type::Symbol, x0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a new Individual Based Mean Field (IBMF) fixed-point node with the specified index and neighbors.
    ...
    # Output
    - `NodeFP_IBMF`: Node with the specified index and neighbors.
    """
    function NodeFP_IBMF(i::I, neighs::Vector{I}, init_type::Symbol, x0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
        if init_type == :zero
            return new{Deterministic, I, RT}(i, neighs, zero(RT))
        elseif init_type == :random
            return new{Deterministic, I, RT}(i, neighs, rand(rng))
        elseif init_type == :custom
            return new{Deterministic, I, RT}(i, neighs, x0)
        end
    end
end
...
$(TYPEDFIELDS)
"""
struct NodeFP_BP{Deterministic, I<:Integer, RT<:Real}
    """ Node index """
    i::I
    """ Neighbors' indices """
    neighs::Vector{I}
    ...
    neighs_idx::Dict{I, I} # Value is the 1-based index in the neighs vector
    """ Cavity messages """
    cavs::Vector{CavityFP_BP{Deterministic, I, RT}}
    """ Marginal """
    marg::MarginalFP_BP{Deterministic, I, RT}
    """
        NodeFP_BP{Deterministic, I, RT}(i::I, neighs::Vector{I}, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a new Belief Propagation (BP) fixed-point node with the specified index and neighbors.
    ...
    # Output
    - `NodeFP_BP`: Node with the specified index and neighbors.
    """
    function NodeFP_BP(i::I, neighs::Vector{I}, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
        neighs_idx = Dict(neighs .=> eachindex(neighs))  # Map neighbor indices to their positions in the `neighs` vector
        cavs = [CavityFP_BP(i, j, init_type, mu0, q0, rng) for j in neighs] # Initialize cavities
        marg = MarginalFP_BP(i, init_type, mu0, q0, rng) # Initialize marginal
        new{Deterministic, I, RT}(i, neighs, neighs_idx, cavs, marg)
    end
end
...
$(TYPEDFIELDS)
"""
struct PopFP_BP{Deterministic, I<:Integer, RT<:Real}
    """Population of averages"""
    mu_pop::Vector{RT}
    """Population of cavity correlations"""
    q_pop::Vector{RT}
    """
        PopFP_BP(P::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}

    Create a population of messages for the Belief Propagation (BP) fixed-point algorithm in deterministic dynamics.
    ...
    # Returns
    - `PopFP_BP{Deterministic, I, RT}`: Population of fixed-point cavity averages.
    """
    function PopFP_BP(P::I, init_type::Symbol, mu0::RT, q0::RT, rng::AbstractRNG) where {I<:Integer, RT<:Real}
        if init_type == :zero
            return new{Deterministic, I, RT}(zeros(RT, P), zeros(RT, P))
        elseif init_type == :random
            return new{Deterministic, I, RT}(rand(rng, RT, P), rand(rng, RT, P))
        elseif init_type == :custom
            return new{Deterministic, I, RT}(fill(mu0, P), fill(q0, P))
        end
    end
end
