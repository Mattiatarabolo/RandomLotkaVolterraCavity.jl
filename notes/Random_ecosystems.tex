...
We use the Gaussian Expansion Cavity method \cite{}
...
\begin{align}
  \frac{d}{dt}x_{i\setminus j}(t) & =x_{i\setminus j}(t)\Biggl[1-x_{i\setminus j}(t)+\sum_{k\in\partial i\setminus j}J_{ik}\mu_{k\setminus i}(t) + \sum_{k\in\partial i\setminus j}\int_{0}^{t}dt'J_{ik}J_{ki}R_{k\setminus i}(t,t')x_{i\setminus j}(t') \nonumber \\
   & \qquad\quad \qquad +h_{i\setminus j}(t)+\zeta_{i\setminus j}(t)\Biggr]+\xi_{i\setminus j}(t)\label{eq:eff_proc_cav}
\end{align}
...
\begin{equation}
  \left\langle \zeta_{i\setminus j}(t)\zeta_{i\setminus j}(t')\right\rangle _{i\setminus j}=\sum_{k\in\partial i\setminus j}J_{ik}^{2}C_{k\setminus i}(t,t'),\qquad\left\langle \xi_{i\setminus j}(t)\xi_{i\setminus j}(t')\right\rangle _{i\setminus j}=g(x_{i\setminus j}(t))\delta(t-t').
\end{equation}
...
\begin{equation}
\mu_{k\setminus i}(t)=\left\langle x_{k\setminus i}(t)\right\rangle _{k\setminus i},\quad R_{k\setminus i}(t,t')=\frac{\delta}{\delta h_{k\setminus i}(t')}\left\langle x_{k\setminus i}(t)\right\rangle _{k\setminus i},\quad C(t,t')=\left\langle x_{k\setminus i}(t)x_{k\setminus i}(t')\right\rangle _{k\setminus i},
\end{equation}
...
\begin{align}
  \frac{d}{dt}x_{i}(t) & =x_{i}(t)\Biggl[1-x_{i}(t)+\sum_{k\in\partial i}J_{ik}\mu_{k\setminus i}(t) + \sum_{k\in\partial i}\int_{0}^{t}dt'J_{ik}J_{ki}R_{k\setminus i}(t,t')x_{i}(t') \nonumber \\
   & \qquad\quad \qquad +h_{i}(t)+\zeta_{i}(t)\Biggr]+\xi_{i}(t),\label{eq:eff_proc_marg}
\end{align}

with Gaussian noises with vanishing mean and correlations

\begin{equation}
  \left\langle \zeta_{i}(t)\zeta_{i}(t')\right\rangle _{i}=\sum_{k\in\partial i}J_{ik}^{2}C_{k\setminus i}(t,t'),\qquad\left\langle \xi_{i}(t)\xi_{i}(t')\right\rangle _{i}=g(x_{i}(t))\delta(t-t').
\end{equation}
...
can be found in Appendix \ref{app:naive_method}. The problem is that this approximation corresponds to a large connectivity expansion, already done by \cite{aguirre-lopezHeterogeneousMeanfieldAnalysis2024}.
...
the derivation of \cite{opperPhaseTransition11992,gallaRandomReplicatorsAsymmetric2006,gallaDynamicallyEvolvedCommunity2018},
...
the derivation of \cite{opperPhaseTransition11992,gallaDynamicallyEvolvedCommunity2018}.
...
\begin{equation}
Z=\left\langle \int\mathcal{D}\vec{x}\mathcal{D}\vec{\hat{x}}\prod_{i}p_{i}\left(x_{i}(0)\right)e^{\int dt\,({-{\rm i}}\hat{x}_{i}(t))\left[\dot{x_{i}}(t)-x_{i}(t)\left(1-x_{i}-\sum_{j\neq i}a_{ij}J_{ij}x_{j}+h_{i}(t)\right)-\eta_i(t)\right]}\right\rangle _{\vec{\eta}}.
\end{equation}
...
\begin{align}
  S_i & = \int dt \left\{-{\rm i}\hat{x}_i(t)\left(\dot{x}_i(t)-x_i(t)f_i(t)\right)-\frac{1}{2}g(x_i(t))\hat{x}_i^2(t)-{\rm i}\hat{f}_i(t)\left[f_i(t)-\left(1-x_i(t)+h_i(t)\right)\right] \right\}\\
...
\begin{align}
  S^{\rm eff}_{i\setminus j} & = \int dt \Bigg\{-{\rm i}\hat{x}_i(t)\left(\dot{x}_i(t)-x_i(t)f_i(t)\right)-\frac{1}{2}g(x_i(t))\hat{x}_i^2(t)-{\rm i}\hat{f}_i(t)\Big[f_i(t)-\left(1-x_i(t)+h_i(t)\right) \nonumber \\
...
\begin{align}
\frac{d}{dt}x_{i\setminus j}(t) & =x_{i\setminus j}(t)\Biggl[1-x_{i\setminus j}(t)+\sum_{k\in\partial i\setminus j}J_{ik}\mu_{k\setminus i}(t) + \sum_{k\in\partial i\setminus j}\int_{0}^{t}dt'J_{ik}J_{ki}R_{k\setminus i}(t,t')x_{i\setminus j}(t') \nonumber \\
 & \qquad\quad \qquad +h_{i\setminus j}(t)+\zeta_{i\setminus j}(t)\Biggr]+\xi_{i\setminus j}(t),
\end{align}
...
\begin{align}
  \frac{d}{dt}x_{i}(t) & =x_{i}(t)\Biggl[1-x_{i}(t)+\sum_{k\in\partial i}J_{ik}\mu_{k\setminus i}(t) + \sum_{k\in\partial i}\int_{0}^{t}dt'J_{ik}J_{ki}R_{k\setminus i}(t,t')x_{i}(t') \nonumber \\
   & \qquad\quad \qquad +h_{i}(t)+\zeta_{i}(t)\Biggr]+\xi_{i}(t),
\end{align}
...
We use the Gaussian Expansion Cavity method \cite{taraboloGaussianApproximationDynamic2025}
to derive a Langevin equation for a representative species. This approach
begins by constructing the Martin-Siggia-Rose-Janssen-De Dominicis
(MSRJD) dynamic partition function through path integrals of the trajectories
\cite{martinStatisticalDynamicsClassical1973,janssenLagrangeanClassicalField1976,dedominicisDynamicsSubstituteReplicas1978}
...
of \cite{aguirre-lopezHeterogeneousMeanfieldAnalysis2024}
...
already done by \cite{aguirre-lopezHeterogeneousMeanfieldAnalysis2024},
.
